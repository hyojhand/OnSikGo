<template>
  <v-app id="app" class="margin">
    ...
    <v-navigation-drawer app v-model="setting" absolute clipped temporary>
      ...
      <v-list v-if="userState === 1" nav>
        <v-list-item v-for="item in settingOwners" :key="item.title" :to="item.router">
          <v-list-item-content>
            <v-list-item-title>{{ item.title }}</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      ...
      <v-list v-else nav>
        <v-list-item v-for="item in settingUsers" :key="item.title" :to="item.router">
          <v-list-item-content>
            <v-list-item-title>{{ item.title }}</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
...
export default {
  ...
  data () {
    return {
      ...
      settingUsers: [
        { title: '회원정보수정', router: '/userinfochange'},
      ],
      settingOwners: [
        { title: '회원정보수정', router: '/ownerinfochange'},
      ]
    }
  },
  updated() {
